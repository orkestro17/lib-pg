...
describe("db.sql.client", () => {
  ...
  describe("run()", () => {
    it("executed query and logs duration", async () => {
      ...
      eq(logs[0][0], "Query(select pg_sleep(0.055)): select pg_sleep(0.055)")
    ...
    it("executes multiple queries", async () => {
      ...
      eq(
        ...
        [
          ...
          "[txn:testTransaction:<uuid>] Query(select 1 as a): select 1 as a",
          "[txn:testTransaction:<uuid>] Query(select 2 as b): select 2 as b",
    ...
    it("releases client on error", async () => {
      ...
      eq(
        ...
        [
          ...
          "[txn:query2:<uuid>] Query(select 2): select 2",
  ...
  describe("getPg stats", () => {
    it("return info about Pg", async () => {
      ...
      const props = ["maxConnections", "pgStatActivity", "pgStatActivity", "pgActiveConnections"]
      console.log(stats)

      props.forEach((p) => expect(stats).has.property(p).with.lengthOf.at.least(1))
    })
  })
...
function replaceRandomValues(args: unknown[]): (string | unknown)[] {
  // replace uuids for easier assertions
  return args.map((value) => (typeof value === "string" ? value.replace(/[0-9a-f-]{36}/g, "<uuid>") : value))
}
