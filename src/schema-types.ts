...
export async function generateSchemaTypeDeclarations(outputFileName: string, pgConfig: Pg.ClientConfig): Promise<void> {
  const db = new Pg.Client(pgConfig)
  await db.connect()
  try {
    const schemaOutput = await getDbSchemaDeclarations(db, "public")
    writeFileSync(outputFileName, header + "\n" + "declare " + formatNamespace("SqlSchema", schemaOutput) + "\n")
  } finally {
    await db.end()
  }
}
...
async function getEnums(client: Pg.ClientBase, schema: string): Promise<Enum[]> {
...
function formatTable({ tableName, columns }: TableInfo, declaredTypes: string[]) {
  const output = []
...
async function getTables(db: Pg.ClientBase, schema: string): Promise<TableInfo[]> {
  type Row = {
    table_name: string
    column_name: string
    data_type: string
  ...
  return keys(groupedByTable)
    ...
    .map((key) => ({
      tableName: key,
      columns: sortBy(groupedByTable[key], (row) => row.column_name).map((row) => ({
