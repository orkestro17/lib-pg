...
async function runSql(queryConfig, ctx = {}) {
  ...
  let { text, name = null } = typeof queryConfig === "string" ? { text: queryConfig } : queryConfig
...
async function transaction(name, ctx = {}, f) {
  ...
  const transactionError = new Error(`Query transaction ${name}[${pgTxnId}] failed`)
  ...
  try {
    ...
    try {
      ...
      log(
        new Error(`Additional error occoured while rolling back a transaction: ${e.stack}\n${transactionError.stack}`)
      )
    }
    throw e
  } finally {
...
async function runSqlFile(fileName, ctx = {}) {
  ...
  for (const row of splitSqlText(content)) {
    await runSql({ text: row.queryText, ignoreErrorCodes: row.ignoreErrorCodes }, ctx)
  }
}

async function runSqlFiles(...args) {
  for (let arg of args) {
    for (let f of await Glob(arg)) {
      await runSqlFile(f)
    }
  }
}
...
async function getPgStats() {
  const maxConnections = await runSql("SELECT * FROM pg_settings WHERE name = 'max_connections'")
  const pgStatActivity = await runSql(`SELECT * FROM pg_stat_activity where datname = '${config.database}'`)
  const pgStatDatabase = await runSql(`SELECT * FROM pg_stat_database where datname = '${config.database}'`)
  const pgActiveConnections = await runSql("SELECT sum(numbackends) FROM pg_stat_database")
