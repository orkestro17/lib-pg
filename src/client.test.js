...
const { pool, runSql, transaction, getPgStats, PoolClient } = require("lib/sql")
const { usingPg } = require("./testing")
const LogMock = require("lib/log.mock")
...
describe("db.sql.client", () => {
  ...
  describe("transaction()", () => {
    it("executes multiple queries", async () => {
      const result = await transaction("testTransaction", { log, pgTxnId: "test-txn" }, (ctx) =>
        Promise.all([runSql("select 1 as a", ctx), runSql("select 2 as b", ctx)])
      )
      eq(result, [[{ a: 1 }], [{ b: 2 }]])
      eq(
        ...
        [
          ...
          [{ pgTxnId: "test-txn" }, "Query transaction testTransaction success"]
        ]
      )
    })
